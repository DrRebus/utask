...
func bindFlags() {
	...
	flags := pflag.NewFlagSet("utask-flags", pflag.ContinueOnError)
	...
	viper.BindPFlag(envInit, flags.Lookup("init-path"))
	viper.BindPFlag(envPlugins, flags.Lookup("plugins-path"))
	viper.BindPFlag(envTemplates, flags.Lookup("templates-path"))
	viper.BindPFlag(envFunctions, flags.Lookup("functions-path"))
	viper.BindPFlag(envScripts, flags.Lookup("scripts-path"))
	viper.BindPFlag(envRegion, flags.Lookup("region"))
	viper.BindPFlag(envHTTPPort, flags.Lookup("http-port"))
	viper.BindPFlag(envDebug, flags.Lookup("debug"))
	viper.BindPFlag(envMaintenance, flags.Lookup("maintenance-mode"))
	viper.BindPFlag(envLogsFormat, flags.Lookup("logs-format"))
}

func InitUtask() (*plugins.Service, error) {
	bindFlags()
	...
	if utask.FPort > 65535 || utask.FPort == 0 {
		return nil, errors.New("Incorrect HTTP port range")
	}

	return service, db.Init(store)
}
